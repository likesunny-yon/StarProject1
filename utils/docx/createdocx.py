...
def createdocx(docxname, profile, path='/'):
    ...
    for language in profile['languages']:
        doc.add_paragraph(language['language']+'-'+language['pro'], style='BodyFormat')

    if not os.path.exists(path):
        print("ERROR: Unvaild docx path inputed.")
        # os.makedirs(path)

    # Save the Word document
    docxpath = os.path.join(path, docxname + '.docx')
...
if __name__ == '__main__':
    ...
    filename = '1'
    with open('../../users/' + filename + '.json', 'r', encoding='utf-8') as f:
        profile = json.load(f)
    createdocx(filename,profile,path='/')
