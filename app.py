...
def create():
    ...
    if (name==""):
        import glob
        profiles = glob.glob(config.Profile_Path + "/*.json")
        names = [os.path.splitext(os.path.basename(file))[0] for file in profiles]

    for i in range(num):
        for name in names:
            os.system('python here.py ' + name)
...
def open():
    ...
    os.system('python utils\\upwork\\new_instance.py ' + email[0] + ' ' + url)
